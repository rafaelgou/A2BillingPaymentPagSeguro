...
getpost_ifset (array('transaction_id'));
...
try {
  $transaction = TransactionSearchService::searchByCode(  
      $credentials,  
      $transaction_id  // 'A08F36E8-F79B-40EF-9837-982E244C848C'
  );  
} catch (Exception $exc) {
  write_log(LOGFILE_EPAYMENT, basename(__FILE__).
    ' line:'.__LINE__." EPAYMENT PAGSEGURO: transaction_id=$transaction_id".
...
if ($transaction)
{
  ...
  $QUERY = "SELECT id, cardid, amount, vat, paymentmethod, cc_owner, cc_number, cc_expires, creationdate, status, cvv, credit_card_type, currency, item_id, item_type " .
       ...
       " WHERE id = ".$transactionID." AND status = 0";
  
  $transaction_data = $paymentTable->SQLExec ($DBHandle_max, $QUERY);

  if(empty($transaction_data) || count($transaction_data) == 0) {
    write_log(LOGFILE_EPAYMENT, basename(__FILE__).
      ' line:'.__LINE__." EPAYMENT PAGSEGURO: transactionID=$transactionID"." ERROR INVALID TRANSACTION ID PROVIDED");
     header('Location: checkout_success.php?errcode=-2');
  } else {
    write_log(LOGFILE_EPAYMENT, basename(__FILE__).
      ' line:'.__LINE__." EPAYMENT PAGSEGURO: transactionID=$transactionID"." EPAYMENT RESPONSE: TRANSACTIONID = ".$transactionID.
  ...
  write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO: curr amount $currAmount $currCurrency ".BASE_CURRENCY);
  ...
  write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line: '.__LINE__." EPAYMENT PAGSEGURO: STATUS UPDATE TO 'In Process'");
...
if ($resmax) {
  $numrow = $resmax -> RecordCount();
    ...
    write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO: transactionID=$transactionID"." ERROR NO SUCH CUSTOMER EXISTS, CUSTOMER ID = ".$transaction_data[0][1]);
...
write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO: transactionID=$transactionID"." status $orderStatus ($pagseguroStatus)");
...
if ($customer_info[0] > 0 && $orderStatus == 2) {
  ...
  write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO: transactionID=$transactionID"." CARD FOUND IN DB ($id)");
} else {
    write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO: transactionID=$transactionID"." ERROR CUSTOMER INFO OR ORDERSTATUS ($orderStatus)\n".print_r($_POST, true)."\n". $Query . "\n");
}
...
if ($id > 0 ) 
{
  if (strcasecmp("invoice",$item_type)!=0) 
  {
    ...
    write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO: transactionID=$transactionID"." Update_table cc_card : $param_update - CLAUSE : $FG_EDITION_CLAUSE");
    ...
    write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO: transactionID=$transactionID"." Add_table cc_logrefill : $field_insert - VALUES $value_insert");
    ...
    write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO: transactionID=$transactionID"." Add_table cc_logpayment : $field_insert - VALUES $value_insert");
    ...
    if (!empty($id_agent)) 
    {
      ...
      if(is_array($result_agent) && is_numeric($result_agent[0]['commission']) && $result_agent[0]['commission']>0) 
      {
        ...
        write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO: transactionID=$transactionID"." Add_table cc_agent_commission : $field_insert - VALUES $value_insert");
        ...
        write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO: transactionID=$transactionID"." Update_table cc_agent : $param_update_agent - CLAUSE : $clause_update_agent");
      }
    }
  } else {
    if($item_id>0) 
      ...
      if (is_array($result_invoice) && sizeof($result_invoice)==1) 
      {
        ...
        foreach ($items as $item) 
        {
          ...
          if ($item -> getExtType() == 'SUBSCR') {
            //Load subscription
            write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO:  Type SUBSCR");
            ...
            if(is_array($result_subscr))
            {
              ...
              write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO: cc_card_subscription paid_status : ".$subscription['paid_status']);
              if($subscription['paid_status']==3){
                ...
                write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO: QUERY : $QUERY - RESULT : $result");
                ...
                write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO: QUERY : $QUERY");
                $instance_table->SQLExec($DBHandle, $QUERY, 0);
              }else{
                $QUERY = "UPDATE cc_card SET status=1 WHERE id=$id";
                $result = $instance_table->SQLExec($DBHandle, $QUERY, 0);
...
write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO: transactionID=$transactionID"." EPAYMENT ORDER STATUS  = ".$statusDescription);
...
if (preg_match("/^[a-z]+[a-z0-9_-]*(([.]{1})|([a-z0-9_-]*))[a-z0-9_-]+[@]{1}[a-z0-9_-]+[.](([a-z]{2,3})|([a-z]{3}[.]{1}[a-z]{2}))$/i", $customer_info["email"])) {
  // FIND THE TEMPLATE APPROPRIATE
    ...
    try {
        ...
        write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO: SENDING EMAIL TO CUSTOMER ".$customer_info["email"]);
        ...
        write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO: SENDING EMAIL TO CUSTOMER ".$customer_info["email"]);
        write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO: transactionID=$transactionID"."- MAILTO:".$customer_info["email"]."-Sub=".$mail->getTitle()." , mtext=".$mail->getMessage());
        ...
        write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO: transactionID=$transactionID"." ERROR NO EMAIL TEMPLATE FOUND");
    }
  
} else {
  write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO: transactionID=$transactionID"." Customer : no email info !!!");
}

write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO: transactionID=$transactionID"." EPAYMENT ORDER STATUS ID = ".$orderStatus." ".$statusDescription);
write_log(LOGFILE_EPAYMENT, basename(__FILE__).' line:'.__LINE__." EPAYMENT PAGSEGURO: transactionID=$transactionID"." ----EPAYMENT TRANSACTION END----");
